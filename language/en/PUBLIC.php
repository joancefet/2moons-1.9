<?php
/**
 *  2Moons
 *   by Jan-Otto Kröpke 2009-2016
 *
 * For the full copyright and license information, please view the LICENSE
 *
 * @package 2Moons
 * @author mvw <mvgadmin@gmail.com>
 * @copyright 2017 2moons
 * @copyright 2017 Jan-Otto Kröpke <slaver7@gmail.com>
 * @licence MIT
 * @version 1.9.0
 * @link https://github.com/HikeGame/2moons-1.9
 */

// Translated into English by mvw. All rights reversed (C) 2017
// 2Moons - Copyright (C) 2010-2017

// Site Title
$LNG['siteTitleIndex']				   = 'Index';
$LNG['siteTitleRegister']			   = 'Registeration';
$LNG['siteTitleScreens']			   = 'Screenshots';
$LNG['siteTitleBanList']			   = 'Banned Hall';
$LNG['siteTitleBattleHall']			 = 'Battle Hall';
$LNG['siteTitleRules']				   = 'Rules';
$LNG['siteTitleNews']				     = 'News';
$LNG['siteTitleDisclamer']			 = 'Contacts';
$LNG['siteTitleLostPassword']		 = 'Lost Password?';

// Menu
$LNG['forum']						         = 'Forum';
$LNG['menu_index']					     = 'Index';
$LNG['menu_news']					       = 'News';
$LNG['menu_rules']					     = 'Rules';
$LNG['menu_banlist']				     = 'Banned Hall';
$LNG['menu_battlehall']				   = 'Battle Hall';
$LNG['menu_disclamer']				   = 'Contacts';
$LNG['menu_register']				     = 'Registration';

// Universe select
$LNG['chose_a_uni']					     = 'Choose Universe';
$LNG['universe']					       = 'Universe';
$LNG['uni_closed']					     = ' (closed)';

// Button
$LNG['buttonRegister']				   = 'Register Now!';
$LNG['buttonScreenshot']			   = 'Screenshots';
$LNG['buttonLostPassword']			 = 'Lost Password?';

// Start
$LNG['gameInformations']    		 = "A space strategy game in real time.\nGame with hundreds of users.\nNo download, it only requires a standard browser.\nFree registration";

// Login
$LNG['loginHeader']					     = 'Login';
$LNG['loginUsername']				     = 'Username';
$LNG['loginPassword']				     = 'Password';
$LNG['loginButton']					     = 'Login';
$LNG['loginInfo']					       = 'By logging in I accept the %s.';
$LNG['loginWelcome']				     = 'Welcome to %s';
$LNG['loginServerDesc']				   = '%s is a space strategy game with hundreds of players playing together <strong>at the same time</strong> trying to control the universe with a mighty empire. You only require a browser to play';

// Register
$LNG['registerFacebookAccount']	 = 'Facebook Account';
$LNG['registerUsername']			   = 'Username';
$LNG['registerUsernameDesc']		 = 'Username must contain at least 3 and maximum of 25 characters, and may only consist of numbers, letters, underscores and spaces';
$LNG['registerPassword']			   = 'Password';
$LNG['registerPasswordDesc']		 = 'The password must be at least 8 characters long!';
$LNG['registerPasswordReplay']	 = 'Repeat Password';
$LNG['registerPasswordReplayDesc']	= 'Enter the same password';
$LNG['registerEmail']				     = 'E-Mail';
$LNG['registerEmailDesc']			   = 'Enter your E-Mail address!';
$LNG['registerEmailReplay']			 = 'Repeat E-Mail';
$LNG['registerEmailReplayDesc']	 = 'Please input of your email address once again to the security!';
$LNG['registerLanguage']			   = 'Language';
$LNG['registerReferral']			   = 'Referred by:';
$LNG['registerCaptcha']				   = 'Prove your human';
$LNG['registerCaptchaDesc']			 = 'Please enter the characters below in the empty field. There does not have case sensitivity';
$LNG['registerCaptchaReload']		 = 'Reload Captcha.';
$LNG['registerRules']				     = 'Rules';
$LNG['registerRulesDesc']			   = 'I agree with the %s';

$LNG['registerBack']				     = 'Back';
$LNG['registerNext']				     = 'Next';

$LNG['registerErrorUniClosed']		  = 'Registration is closed for this universe!';
$LNG['registerErrorUsernameEmpty']	= 'You must enter a username!';
$LNG['registerErrorUsernameChar']	  = 'Your username must consist of numbers, Letters, Spaces, _, -, . only!';
$LNG['registerErrorUsernameExist']	= 'Username already taken!';
$LNG['registerErrorPasswordLength']	= 'The password must be at least 8 characters long!';
$LNG['registerErrorPasswordSame']	  = 'Passwords do not match';
$LNG['registerErrorMailEmpty']		  = 'You must specify a E-Mail address!';
$LNG['registerErrorMailInvalid']	  = 'Invalid E-Mail address entered!';
$LNG['registerErrorMailSame']		    = 'E-Mail Addresses do not match!';
$LNG['registerErrorMailExist']		  = 'E-Mail address already registered!';
$LNG['registerErrorRules']			    = 'You have to accept the rules!';
$LNG['registerErrorCaptcha']		    = 'We could not verify that you are human';

$LNG['registerMailVertifyTitle']	  = 'Game Registration Activation required for: %s';
$LNG['registerMailVertifyError']	  = 'Failed to send E-Mail to: %s';

$LNG['registerMailCompleteTitle']	  = 'Welcome to %s!';

$LNG['registerSendComplete']		    = 'Thank you for registering. Check your E-Mail (also check the Spam Folder) for further information';

$LNG['registerWelcomePMSenderName']	= 'Administrator';
$LNG['registerWelcomePMSubject']	  = 'Welcome';
$LNG['registerWelcomePMText']		    = 'Welcome to %s! </br>In order to start your empire, build a solar power plant first, as energy is needed for the production of raw materials. To build one, click on "Buildings" from the menu to the left. Then build the Solar Power Plant. </br> When you have energy, you can begin to use built mines. Go to buildings and build a metal mine, followed by a crystal mine. </br></br>In order to be able to build ships you need to have a shipyard. To see what is needed to unlock that building you can take a look at the "Technologies" section.</br>If you more questions you can look at the beginners guide, our forum or open a support ticket. </br></br> The Admin team wishes you well in your journey exploring the universe!';

//Verify

$LNG['vertifyNoUserFound']			= 'Invalid Request!';
$LNG['vertifyAdminMessage']			= 'The username "%s" has enabled!';


//lostpassword
$LNG['passwordInfo']				        = 'If you have forgotten your password, you must specify the user name and the E-Mail address that you have entered in your account.';
$LNG['passwordUsername']			      = 'Username';
$LNG['passwordMail']				        = 'E-Mail';
$LNG['passwordCaptcha']				      = 'Security code';
$LNG['passwordSubmit']				      = 'Submit';
$LNG['passwordErrorUsernameEmpty']	= 'You have no specified the Username!';
$LNG['passwordErrorMailEmpty']		  = 'You have specified an unknown email address!';
$LNG['passwordErrorUnknown']		    = 'It could not be found with the account data.';
$LNG['passwordErrorOnePerDay']		  = 'Password for this user account in the last 24 hours previously requested. You can only request the password for this user account in  next 24 hours. ';

$LNG['passwordValidMailTitle']		  = 'Forgot my password on the game: %s';
$LNG['passwordValidMailSend']		    = 'You will receive an email shortly with further information.';

$LNG['passwordValidInValid']		    = 'There is an outstanding Password Request for this account, check your SPAM folder';
$LNG['passwordChangedMailSend']		  = 'You will receive an email shortly with the new password';
$LNG['passwordChangedMailTitle']	  = 'New password on the game: %s';

$LNG['passwordBack']				        = 'Back';
$LNG['passwordNext']				        = 'Next';

//case default

$LNG['login_error_1']				       = 'Wrong Username / Password!';
$LNG['login_error_2']				       = 'Someone has logged into your account from a different location, or your IP has changed since your last login!';
$LNG['login_error_3']				       = 'Your session has expired!';
$LNG['login_error_4']				       = 'There was an error with the external authorization service, please try again!';

//Rules
$LNG['rulesHeader']				     	   = 'Rules';

//NEWS
$LNG['news_overview']				       = 'News';
$LNG['news_from']					         = 'On %s by %s';
$LNG['news_does_not_exist']			   = 'No news available!';

//Impressum
$LNG['disclamerLabelAddress']		   = 'Game Address:';
$LNG['disclamerLabelPhone']			   = 'Telephone Number:';
$LNG['disclamerLabelMail']			   = 'Support E-Mail:';
$LNG['disclamerLabelNotice']		   = 'More information';
